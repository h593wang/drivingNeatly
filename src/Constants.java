public class Constants {
    public static final float C1 = 1.0f;
    public static final float C2 = 1.0f;
    public static final float C3 = 0.4f;
    public static final float DT = 4.0f;
    public static final float MUTATION_RATE = 0.8f;
    public static final float ADD_CONNECTION_RATE = 0.1f;
    public static final float ADD_NODE_RATE = 0.005f;
    public static final int MAX_STAGNATION = 15;
    public static final float STAGNATION_DECAY = 0.08f;

    //Genome
    public static final float PROBABILITY_PRETURBING = 0.9f;

    //Car
    public static final int INACTIVE_COUNT = 300;
    public static final float ACTIVATION_THRESHOLD = 0.5f;
    public static final int FRAME_RATE_FAST = 1;
    public static final int FRAME_RATE_SLOW = 40;
    public static final int FOV = 120;
    public static final int SENSOR_COUNT = 5;
    public static final boolean WITH_W_S = false;
    public static final float MAX_FITNESS = 3740;
}
