...
public abstract class NeuralNetwork {
    ...
    public NeuralNetwork(Genome genome) {
        ...
        for (Integer nodeID : genome.nodes.keySet()) {
            NodeGene node = genome.nodes.get(nodeID);
        ...
        for (Integer connID : genome.connections.keySet()) {
            ConnectionGene conn = genome.connections.get(connID);
            if (!conn.enabled) {
                continue;
            }
            Neuron inputter = neurons.get(conn.inNode); // the node that leads into the connection, is the inputter to the connection
            ...
            Neuron outputReceiver = neurons.get(conn.outNode); // the node that receives from the connection, is the output receiver of the connection
